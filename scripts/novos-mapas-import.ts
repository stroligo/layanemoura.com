/**
 * Importa mapas novos de `NOVOS MAPAS/` → projeto no site.
 *
 *   npm run mapas:import-novos
 *   npm run mapas:import-novos -- --dry-run
 *
 * Por projeto (pastas ou ficheiros com o mesmo título):
 *   1. Converte originais → public/images/projects/{slug}/NN.webp
 *   2. Gera thumb + display (`optimize-images --only=slug`)
 *   3. Cria ou atualiza content/projects/{slug}.yml
 *   4. Apaga os ficheiros processados em `NOVOS MAPAS/`
 *
 * Nomenclatura (igual a MAPAS/):
 *   `Título - Subtítulo 2.png`  → subtítulo + imagem 2
 *   `The Island 1.jpg`          → título The Island, imagem 1
 */
...
const sourceDir = join(root, 'NOVOS MAPAS');
...
function scanSourceFiles(): SourceFile[] {
  if (!existsSync(sourceDir)) {
    console.error(`Pasta em falta: ${sourceDir}`);
    console.error('Crie "NOVOS MAPAS" e coloque lá os ficheiros JPG/PNG.');
    process.exit(1);
  }
  ...
  if (!files.length) {
    console.log('Nenhum JPG/PNG em NOVOS MAPAS/.');
    process.exit(0);
  }
...
async function main() {
  ...
  if (!dryRun) {
    ...
    console.log('\nA remover originais em NOVOS MAPAS/…');
    ...
    console.log('[dry-run] Apagaria os ficheiros em NOVOS MAPAS/ após sucesso.');
  }
